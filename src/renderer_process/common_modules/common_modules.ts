...
export const replaceFullWidthNumToHalfWidthNum = (args: { element: HTMLInputElement, limitDigits?: number }): void => {
    ...
    const replaceNumbers = (): void => {
        let inputtedValue: string = String(element.value);
        ...
        if (limitDigits && inputtedValueLength > limitDigits) {
            inputtedValue = inputtedValue.slice(0, limitDigits);
        }

        element.value = inputtedValue;
    }
}
